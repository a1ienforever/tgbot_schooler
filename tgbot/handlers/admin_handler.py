...
from tgbot.misc.states import InputFile
...
@role_required(["director"])
async def test(message: Message, user: TgUser, state: FSMContext):
    await state.set_state(InputFile.input)
    await message.answer(
        "Для импорта учащихся пожалуйста отправьте файл в формате .xlsx\n"
        "Со следующими полями ФИО, КЛАСС, Буква, Корпус"
    )


@router.message(F.document, InputFile.input)
