...
class CallbackSpamMiddleware(BaseMiddleware):
    ...
    async def __call__(
        ...
        self.log(
            f"Received callback: user_id={user_id}, callback_prefix={callback_prefix}"
        )
        ...
        if user_storage.get(callback_prefix, False):
            self.log(
                f"Callback with prefix {callback_prefix} for user_id={user_id} is already active, ignoring."
            )
            return
        ...
        self.log(
            f"Callback with prefix {callback_prefix} for user_id={user_id} marked as active."
        )
        ...
        try:
            ...
            self.log(
                f"Error while handling callback for user_id={user_id}, prefix={callback_prefix}: {e}"
            )
            ...
            self.log(
                f"Callback with prefix {callback_prefix} for user_id={user_id} marked as inactive."
            )
